	  ...
	  GeometryReader { geo in
		 ...
		 ZStack {
			ForEach(0..<boxCount, id: \.self) { i in
			   ...
			   // CHANGE: Correct heading calculation for physical compass orientation
			   // When heading is 90° (East), boxes at 90° (right side) should be highlighted
			   let delta = abs((angleDeg + 90 - heading)
...
struct CompassYellowRing_Previews: PreviewProvider {
   static var previews: some View {
	  ...
	  VStack(spacing: 20) {
		 CompassYellowRing(boxCount: 60, boxSize: 6, heading: 0, highlightRange: 45)
			.frame(width: 200, height: 200)

//		 CompassYellowRing(boxCount: 60, boxSize: 6, heading: 90, highlightRange: 45)
//			.frame(width: 200, height: 200)
//
//		 CompassYellowRing(boxCount: 60, boxSize: 6, heading: 180, highlightRange: 45)
//			.frame(width: 200, height: 200)
	  }
	  .padding()
