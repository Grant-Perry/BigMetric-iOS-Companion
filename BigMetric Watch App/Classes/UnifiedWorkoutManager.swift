...
class UnifiedWorkoutManager: NSObject,
   ...
   private let minAcceptableAccuracy: CLLocationAccuracy = 30
   private let minMovementDelta: CLLocationDistance = 5
   private var lastStepCountForDistance: Int = 0 // Track last step tally
   ...
   func locationManager(_ manager: CLLocationManager, didChangeAuthorization status: CLAuthorizationStatus) {
	  ...
	  switch status {
		 case .authorizedAlways, .authorizedWhenInUse:
			// All good, you may want to proceed with location tracking
			return
		 case .notDetermined:
			manager.requestWhenInUseAuthorization()
		 case .denied, .restricted:
			self.logAndPersist("[UWM] Location permission denied/restricted via delegate.")
		 @unknown default:
   ...
   func requestHKAuth() {
	  ...
	  healthStore.requestAuthorization(toShare: typesToShare, read: typesToRead) { [weak self] success, err in
		 DispatchQueue.main.async {
			...
			if let e = err {
			   self?.logger.error("HealthKit auth error: \(e.localizedDescription, privacy: .public)")
   ...
   func stopAndFinish() {
	  ...
	  DispatchQueue.main.asyncAfter(deadline: .now() + 5.0) { [weak self] in
		 ...
		 if self.isSavingToHealthKit && self.shouldFinishWorkoutAfterSessionEnd {
			self.logger.info("[UWM] Safety timeout reached - forcing workout completion")
   ...
   func finishWorkoutAndRoute() {
	  guard !workoutFullySaved else {
		 self.logger.info("[UWM] Workout already saved, ignoring duplicate finishWorkoutAndRoute call")
		 ...
		 do {
			...
			if locationName == "Unknown" || weatherTemp == "--" || weatherSymbol.isEmpty {
			   if let lastLocation = locationsArray.last {
				  ...
				  if locationName == "Unknown" {
					 await withCheckedContinuation { continuation in
						self.geoCodeHelper.getCityNameFromCoordinates(
			...
			if let routeBuilder = self.routeBuilder {
			   ...
			   if let distance = workout.totalDistance?.doubleValue(for: .meter()) {
				  // Optionally update your distance property to match the final saved value
				  DispatchQueue.main.async {
					 self.distance = distance / self.metersToMiles
				  }
			   }
   ...
   private func buildMetadataDictionary() -> [String: Any] {
	  let cleanMeters = sanitizeLocationsAndDistance(locations: self.locationsArray)
	  let finalDistance = cleanMeters / metersToMiles
   ...
   public func locationManager(_ manager: CLLocationManager, didUpdateLocations newLocs: [CLLocation]) {
	  ...
	  if shouldInsertRouteData {
		 let stepsNow = workoutStepCount
		 guard stepsNow > lastStepCountForDistance else {
			// User hasn’t taken any more steps since last update
			// Don’t add GPS drift
			return
		 }
		 ...
		 for location in newLocs {
			guard location.horizontalAccuracy > 0,
				  location.horizontalAccuracy < minAcceptableAccuracy else {
			   continue
			}

			// Filter delta distance (no micro-jitter)
			if let last = lastLocation {
			   let delta = location.distance(from: last)
			   if delta < minMovementDelta {
				  continue
			   }
		 ...
		 // Only update distance if steps advanced
		 distance = totalDistanceAccumulated / metersToMiles
		 lastStepCountForDistance = stepsNow
	  ...
	  activityManager.startActivityUpdates(to: .main) { [weak self] activity in
		 ...
		 if activity.stationary {
			if self.userDeclinedCurrentActivity {
			   ...
			   self.logger.info("[UWM] ✅ User inactivity explicitly detected. Resetting declined activity flag.")
			}
			return
		 }
   ...
   func toggleWalkingTrigger(_ isOn: Bool) {
	  ...
	  if !isOn {
		 clearBufferedLocations()
		 activityManager.stopActivityUpdates()
		 self.logger.info("[UWM] 🚫 Walking trigger explicitly turned OFF. Buffered locations cleared.")
	  } else {
		 startMonitoringActivity()
   ...
   // MARK: - sanitizeLocationsAndDistance function
   func sanitizeLocationsAndDistance(locations: [CLLocation]) -> CLLocationDistance {
	  guard locations.count > 1 else { return 0 }
	  var last: CLLocation? = nil
	  var cleanDistance: CLLocationDistance = 0
	  for loc in locations {
		 guard loc.horizontalAccuracy > 0,
			   loc.horizontalAccuracy < minAcceptableAccuracy else {
			continue
		 }
		 if let l = last {
			let d = loc.distance(from: l)
			if d >= minMovementDelta {
			   cleanDistance += d
			}
		 }
		 last = loc
